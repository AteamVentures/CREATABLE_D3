...
#define PRE_EXTRUDE_X_POS -130


#define MFG_TEST      // Add M1000 command
...
extern unsigned long heatCount;
extern int isHeat;
void setup()
{
  setup_killpin();
  ...
  DDRJ = 0x00;
  if ((PINJ & (1<<PINJ7)) == 0) 
    Enable_Mfg_Test = true;
  else 
    Enable_Mfg_Test = false;    
  ...
  if(\
    preheatHPBTemp < 30 || preheatHPBTemp >150 \
    || preheatHotendTemp <100 || preheatHotendTemp > 270 \
    || heat_hold_minute <1 || heat_hold_minute > 60 \
    || (firstTimeRunPhase != 0 && firstTimeRunPhase != 4)\
    || (firstTimeRunPhase == 0)\
    ){
    // preheatHotendTemp = 200;
    // preheatHPBTemp = 70;
    Config_ResetDefault();
    Config_StoreSettings();
  }

  if(heat_hold_minute <1 || heat_hold_minute > 60){
    heat_hold_minute = 10;
  }
...
#ifdef LED_STRIP  
  set_led_strip_color(128,128,128); // white
#endif
  ...
  while( !card.eof()  && buflen < BUFSIZE && !stop_buffering) {
       ...
       serial_count >= (MAX_CMD_SIZE - 1)||n==-1)
    {
      if(card.eof()){
        ...
        // lcd_implementation_draw_string(2, time);
        // LCD_MESSAGEPGM("Printing Complete ! ");

#ifdef LED_STRIP_
        for (int i = 0; i < 100; i++) {
            set_led_strip_color(255,0,0); // Red
            delay(30);
            set_led_strip_color(0,0,255); // Blue
            delay(30);
        }
            set_led_strip_color(128,128,128); // White
#endif  
...
void process_commands()
{
    ...
    case 29: // G29 Detailed Z-Probe, probes the bed at 3 or more points.
        {
            ...
            // yGap = bed_level[1][1] - bed_level[1][0];
            // bed_level[0][0] = bed_level[1][0] - xGap;
            // bed_level[2][0] = bed_level[1][0] + xGap;
            // bed_level[0][2] = bed_level[0][1] + yGap;
            // bed_level[1][2] = bed_level[1][1] + yGap;
  ...
  else if(code_seen('M'))
  {
    ...
    {// M109 - Wait for extruder heater to reach target.
      ...
      target_direction = isHeatingHotend(tmp_extruder); // true if heating, false if cooling
        ...
        if (Cancle_heating) LCD_MESSAGEPGM(MSG_HEATING_CANCLED);
  ...
  else if(code_seen('V')){      // V Code is custom GCODE
    switch( (int)code_value() ){
      case 28:  // V28 (CLEAN NOZZLE)
         {
      /************************************************************/ //G28
        #ifdef ENABLE_AUTO_BED_LEVELING
#ifdef ACCURATE_BED_LEVELING
      ...
      /************************************************************/ //G92 E0
      current_position[E_AXIS] = 0;//code_value();
      plan_set_e_position(current_position[E_AXIS]);
      /************************************************************/ //G92 E0


      /************************************************************/ //M104 S_nozzleTemp
      setTargetHotend(preheatHotendTemp, tmp_extruder);
      setWatch();
      /************************************************************/ //M104 S_nozzleTemp
      ...
      /************************************************************/ //G1 X{PRE_EXTRUDE_X_POS}
        destination[X_AXIS] = PRE_EXTRUDE_X_POS;
        prepare_move();
      /************************************************************/ //G1 X{PRE_EXTRUDE_X_POS}
      /************************************************************/ //G1 Z1
      ...
      /************************************************************/ //G92 E-32
      current_position[E_AXIS] = -60;//code_value();
      plan_set_e_position(current_position[E_AXIS]);
      /************************************************************/ //G92 E-32
      ...
      /************************************************************/ //G1 X-129 F1000
        ...
        feedrate = 4000;
      /************************************************************/ //G1 Y10 F1000 => Y-10 F1000 3 Cycle
        for(int i = 0 ; i <3 ;i++){
      ...
      /************************************************************/ //ZigZag
        ...
        destination[X_AXIS] = -132;
        prepare_move();

        destination[Y_AXIS] = -8;
        prepare_move();
        
        destination[X_AXIS] = -124;
        prepare_move();

        destination[Y_AXIS] = -6;
        prepare_move();

        destination[X_AXIS] = -132;
        prepare_move();

        destination[Y_AXIS] = -4;
        prepare_move();
        
        destination[X_AXIS] = -124;
        prepare_move();

        destination[Y_AXIS] = -2;
        prepare_move();
      ...
      /************************************************************/ //G28
        #ifdef ENABLE_AUTO_BED_LEVELING
#ifdef ACCURATE_BED_LEVELING
        tempBedLevel[ACCURATE_BED_LEVELING_POINTS][ACCURATE_BED_LEVELING_POINTS];
        for(int i = 0 ; i < ACCURATE_BED_LEVELING_POINTS; i++){
            for(int j = 0 ; j < ACCURATE_BED_LEVELING_POINTS ; j++){
                tempBedLevel[i][j] = bed_level[i][j];
            }
      ...
      /************************************************************/ //G92 E-32
      current_position[E_AXIS] = -32;//code_value();
      plan_set_e_position(current_position[E_AXIS]);
      /************************************************************/ //G92 E-32
...
static int digital_io_test(int pin)
{
  int error_flag = 0;
...
static void do_motor_test()
{
  ...
  calculate_delta(current_position);
  plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], 200, active_extruder);
  ...
  calculate_delta(current_position);
  plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], 500, active_extruder);
...
void Board_Test()
{
  ...
  // Limit S/W - Input  
  send_P(PSTR("1. Limit switch\r\n"));

  if (digital_io_test(X_MAX_PIN) > 0) send_P(PSTR("Y Error\r\n"));
  else send_P(PSTR("Y OK\r\n"));

  if (digital_io_test(Y_MAX_PIN) > 0) send_P(PSTR("Z Error\r\n"));
  else send_P(PSTR("Z OK\r\n"));

  if (digital_io_test(Z_MAX_PIN) > 0) send_P(PSTR("X Error\r\n"));
  else send_P(PSTR("X OK\r\n"));


  // Thermistor - Input
  // Nozzle, Bed - 10K ohm required.
  send_P(PSTR("\r\n2. Thermistor\r\n"));
  ...
  send_P(PSTR("\r\n4. Nozzle & Bed set to 100 dgree\r\n"));
  // Nozzle
  setTargetHotend(100, 0);
  // bed
  setTargetBed(100);


  pinMode(X_MAX_PIN, OUTPUT);
  digitalWrite(X_MAX_PIN, 0);

  pinMode(Y_MAX_PIN, OUTPUT);
  digitalWrite(Y_MAX_PIN, 0);

  pinMode(Z_MAX_PIN, OUTPUT);
  digitalWrite(Z_MAX_PIN, 0);
...
// void customTune(float freq, float duration){
//   float microSec;
//   int repeat;
